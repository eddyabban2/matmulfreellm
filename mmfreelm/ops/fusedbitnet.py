...
def unpack_weights(packed: torch.Tensor, dtype: torch.dtype) -> torch.Tensor:
    table = UNPACK_TABLE.to(packed.device)
    ...
    if packed.ndim == 1:
        # [N,4] -> [4N]
        unpacked = decoded.transpose(0, 1).reshape(-1)
    else:
        # [R,C,...,4] -> [4R,C,...]
        perm = (decoded.ndim - 1,) + tuple(range(decoded.ndim - 1))
        unpacked = decoded.permute(perm).reshape(
            packed.shape[0] * VALUES_PER_ITEM,
            *packed.shape[1:],
        )
